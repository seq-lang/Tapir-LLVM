        ...
        if (BB.size() == 1 && isa<ReattachInst>(BB.getTerminator())) {
          ...
          if (count == 2) { // only predecessors are det.achd and if.then
            for (BasicBlock *Pred : predecessors(&BB)) {
              if (Pred->size() == 2 &&
                  isa<BranchInst>(
                      Pred->getTerminator())) { // if clause only compares
                                                // register contents
                ...
                for (BasicBlock *PredPred : predecessors(Pred)) {
                  if (DetachInst *DI = dyn_cast<DetachInst>(
                          PredPred->getTerminator())) { // outer spawn
                    ...
                    BranchInst *replaceDetach = BranchInst::Create(
                        Pred, end, ((BranchInst *)temp)->getCondition());
                    ReplaceInstWithInst(PredPred->getTerminator(),
                                        replaceDetach);

                    // detach now goes straight to body
                    DetachInst *newDetach =
                        DetachInst::Create(body, end, SyncRegion);
...
static RegisterPass<SpawnUnswitch> X("spawnunswitch", "Do SpawnUnswitch pass",
                                     false, false);

// Public interface to the RedundantSpawn pass
FunctionPass *llvm::createSpawnUnswitchPass() { return new SpawnUnswitch(); }
