  ...
  bool runOnFunction(Function &F) override {
    ...
    do {
      ...
      for (BasicBlock &BB : F) {
        ...
        if (isa<ReattachInst>(BB.getTerminator()) && count < threshold &&
            prior != b) {
          // b ensured to be the corresponding reattach
          effective = true;
          prior = b;
          BranchInst *replaceReattach =
              BranchInst::Create(BB.getSingleSuccessor());
          BranchInst *replaceDetach =
              BranchInst::Create(b->getTerminator()->getSuccessor(0));
...
static RegisterPass<SmallBlock> X("smallblock", "Do SmallBlock pass", false,
                                  false);

// Public interface to the SmallBlock pass
FunctionPass *llvm::createSmallBlockPass() { return new SmallBlock(); }
