        ...
        while (true) {
          ...
          if (isa<DetachInst>(current->getTerminator()) &&
            ...
            BranchInst *replaceReattach =
                BranchInst::Create(start->getSingleSuccessor());
            BranchInst *replaceDetach =
                BranchInst::Create(current->getTerminator()->getSuccessor(0));
...
static RegisterPass<RedundantSpawn> X("redundantspawn",
                                      "Do RedundantSpawn pass", false, false);
